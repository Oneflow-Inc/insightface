...
log_folder=20211014-logs-${MODEL}-${LOSS}/torch_1.10/bz${BZ_PER_DEVICE}/${NODE_NUM}n${gpu_num_per_node}g
mkdir -p $log_folder
...
if [ "$MODEL_PARALLEL" = "True" ] ; then
    ...
    --master_port=1234 train.py configs/speed.py  2>&1 | tee ${log_file} 
